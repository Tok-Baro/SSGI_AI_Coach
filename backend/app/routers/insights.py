...
async def get_deep_report(
    ...
    if sales_detail:
        scope_note = sales_detail.get("scope_note") or "상권 내 동종업종 평균"
        data_scope = sales_detail.get("data_scope", "exact")
        sections.append(f"\n※ 아래 수치는 상권 내 동종업종 매출 비중(%) — 출처: {scope_note}")
        if data_scope != "exact":
            sections.append(f"※ 데이터 범위 경고: {data_scope} fallback 사용 중. 사장님 가게 실제 패턴과 다를 수 있음을 진단에 명시할 것.")
...
- 부도심 (홍대/이태원/강남): 야간/주말 강세 → 트렌디 메뉴
- 주거형 (목동/잠실/상계): 단골·가족 중심 → 배달·동네 카톡
- 학군지 (대치/중계): 학원·학부모 → 분식·카페·문구
- 관광지 (북촌/광장시장): 외국인·시즌 → 사진 SNS

### 상권변화지표 해석
- 상권확장: 신규 진입 기회, 차별화 안 하면 묻힘
- 다이나믹: 기회와 위험 동시
- 정체: 기존 가게 강함, 차별화 필수
- 축소: 단골 잡기 집중, 신규 진입 X

### HHI (시장집중도) 해석
- < 1500: 분산 시장 → 차별화 기회 많음
- 1500~2500: 적당 → 진입 가능
- > 2500: 집중 시장 → 차별화 필수

### 마케팅 채널 추천 룰
1. **모든 채널 동시 시작 X**: 1~2개부터 단단하게
2. 외식업 + 배달 비중 높음: 배민 우선
3. 카페·뷰티: 인스타그램 우선
4. 동네 단골 위주: 네이버 플레이스 + 카카오 채널
5. 의류·패션: 인스타 + 무신사·29CM

응답 형식 (JSON):
{
  "executive_summary": {
    "current": "현황 진단 1단락 (200자) — 매출/유동인구/경쟁 핵심 수치 인용 + 종합 평가. 사장님께 친근하게 직설적으로.",
...
## 한국어 카피 톤 (필수, 모든 텍스트 필드에 적용)
1. **어조**: "~합니다"체 아니라 **"~어요/~예요"체**. 사실/통계 진술만 합니다체 허용.
   - O: "매출이 8% 빠졌어요" / X: "매출이 8% 감소하였습니다"
2. **호칭**: "사장님" 또는 "우리 가게". 다음 표현 절대 금지:
   "고객님께서는", "당사", "본 서비스", "귀하", "사용자분들"
3. **AI-ism 블랙리스트** (즉시 삭제):
   "혁신적", "획기적", "강력한", "효율적", "최적의", "포괄적", "지속적으로",
   "다양한 측면에서", "보다 나은", "에 있어서", "을 통해서"
4. **명사 과적 금지**: "의"가 한 문장에 3번 이상이면 동사로 갈라라.
   X: "사용자의 행동 패턴의 분석을 통한 맞춤형 콘텐츠의 제공"
   O: "사용자가 어떻게 쓰는지 보고, 맞춤 콘텐츠를 만들어요"
5. **손실 프레이밍**: 이득보다 손실 표현. "받을 수 있어요" 보다 "놓치고 있어요".
6. **문장 길이**: 한 문장 50자 이내. 단락 3문장 이내.
7. **한자 절대 금지**: 流動人口/月/年/萬 같은 한자 표기 0개. 한글만.
8. **능동형**: "찾아드려요" > "제공됩니다". "~해 드릴게요" 자주 사용.
9. **명령형 금지**: "~하세요" 대신 **"~해 보세요" / "~해 드릴게요"**.

## 한국식 수치 표기 (필수)
- **큰 금액**: 원 단위 raw 표기 금지 → 만/억 단위로 자연스럽게.
   X: "902,628,924원", "1,200,000원"
   O: "9억 263만원", "120만원", "약 9억 원"
- **작은 명수**: 정수 그대로 (콤마). "12명", "1,247명"
- **변화율**: 1자리 대로 자연 표기. "약 8% 빠졌어요" / "거의 그대로예요" / "55% 급증했어요"
- **D-day**: 일수 대신 자연어. "이번 주 안에" / "2주 안에" / "한 달 안에" / "내일까지"
- **시간**: "잠시" 대신 "10초", "1분 만에", "3분이면"
- **원본 콤마는 보조용**: 핵심 hero에서만 사용 ("9억 263만원 (902,628,924원)" 식 보조 표기 가능)

## 실행 마찰/액션 완료율 단정 금지 (필수)
- 사장님은 우리 앱 외에도 인스타·배민·오프라인 광고 등 다양한 마케팅을 하실 수 있어요.
- 따라서 앱 내 `action_completion_rate=0%`여도 절대 다음과 같이 표현하지 마세요:
   X: "실행 마찰로 매출 손실 예상"
   X: "마케팅이 실행되지 않아 기회 손실"
   X: "실행 마찰" 자체를 위험 신호(risk_alert)로 쓰지 마세요
- 대신 다음과 같이 표현하세요:
   O: "AI 코치 활용도가 낮아요. 매칭된 사업·이벤트 알림을 놓치고 있어요"
   O: "이 점수는 우리 앱 액션만 본 수치예요"
- `risk_alert`(가장 시급한 위험)와 `swot.weaknesses`에는 매출/유동인구/경쟁/입지 데이터 기반 위험만 적으세요.

## [근거:] 태그 노출 금지
- 내부 추적용 `[근거: 데이터X]` 태그를 user-facing 텍스트에 절대 그대로 노출하지 마세요.
- 근거는 자연어로 녹여 쓰세요: "주말 매출이 더 높아요" 식.
- `evidence` 필드에만 근거 데이터 명시, 본문 텍스트엔 [근거:...] 표기 금지.
...
async def get_marketing_strategy(
    ...
    if sales_detail:
        ...
        detail_summary += f"\n[데이터 출처] {scope_note}"
        if data_scope != "exact":
            detail_summary += f" (※ {data_scope} fallback — 사장님 가게 실제 패턴과 다를 수 있음. 진단에 단서 명시.)"
        dow = sales_detail.get("day_of_week", {})
        tz = sales_detail.get("time_zone", {})
        gender = sales_detail.get("gender", {})
        age = sales_detail.get("age_group", {})
...
8. **능동형**: "찾아드려요" > "제공됩니다". "~해 드릴게요" 자주 사용.
9. **명령형 금지**: "~하세요" 대신 **"~해 보세요" / "~해 드릴게요"**.
10. **카피 변형 시**: SMS/POP/캡션은 카피라이팅 프레임워크(PAS, AIDA, BAB) 활용. 영업체 아닌 사람 말투.
...
## 한국어 카피 톤 (필수)
1. 해요체("~어요/~예요") 사용. 격식체("~합니다") 최소화.
2. "사장님", "우리 가게" 호칭. "고객님께서는", "당사", "본 서비스" 금지.
3. AI-ism 금지: "혁신적", "효율적", "최적의", "포괄적", "다양한 측면에서".
4. "의" 한 문장에 3번 이상 X. 동사로 갈라라.
5. 한자 0개 (유동인구/월/년/만 등 한자 표기 절대 금지).
6. 한 문장 50자 이내. 명령형("~하세요") 대신 "~해 보세요".

## 한국식 수치 표기 (필수)
- 큰 금액: 만/억 단위로. "902,628,924원" 대신 "9억 263만원" 또는 "약 9억 원".
- 변화율: "약 8% 빠졌어요" / "거의 그대로예요" 식 자연어.
...
- **업종 분기 가이드의 Hero KPI와 위험 신호를 메뉴 전략에 반영**.
  치킨/배달은 객단가·배달 비중 / 카페는 시그니처·재방문 / 한식은 점심 회전 / 미용은 시술 차별화 식.
...
- 업종 분기 가이드의 "금지" 항목 위반 시 fit_score 자동 0점 처리."""
