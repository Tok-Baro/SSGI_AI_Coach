...
def build():
    ...
    _txt(s, PX(80), PX(96), PX(432), PX(96), "2026 서울시 빅데이터 경진대회\n창업부문",
    ...
    _txt(s, PX(80), PX(1024), PX(432), PX(36), "제품·서비스 부문 · 상세기획서",
         size=8.5, color=C("4A5878"))
    # 우측 메인
    _txt(s, PX(640), PX(196), PX(1200), PX(32), "동네 가게를 위한 AI 경영코치",
    ...
    bignum(s, 640, 868, 380, "라이브 기능", "10", "개", "지금 실제 작동 중인 기능", color=INK)
    bignum(s, 1060, 868, 380, "공공데이터", "10", "종", "열린데이터광장 8 · 포털 2", color=BLUE)
    bignum(s, 1480, 868, 380, "유료 사용자", "0", "명", "베타 · 트랙션 부풀리지 않습니다", color=INK)
    ...
    toc = [
        ...
        ("06", "활용 공공데이터 — 열린데이터광장 8 + 공공데이터포털 2", None, ("공공·AI 20", BLUE, BLUEBG)),
    ...
    contrib = [
        ("데이터 → 의사결정", "공공데이터 10종(열린데이터광장 8 + 공공데이터포털 2)을 한 화면에 결합·해석. \"열려는 있지만 안 쓰이던\" 데이터를 실제 경영 판단으로 연결."),
    ...
    content_head(s, "03 · 활용 공공데이터",
                 [("열린데이터광장 ", False), ("8종", True), (" + 공공데이터포털 ", False), ("2종", True), (" — 모두 라이브 호출 중.", False)],
                 "상권↔행정동 매핑(영역-상권 데이터셋)으로 \"우리 동네\"만 골라냅니다.",
                 rubric="공공·AI 20", title_w=1700)
    # 표 헤더
    hx = [96, 254, 990, 1200, 1620]
    hcols = ["출처", "데이터셋명", "제공기관", "우리 앱 활용", "갱신"]
    ...
    rows = [
        ("열린데이터광장", "서울시 상권분석서비스(추정매출-상권)", "서울신용보증재단", "위험도·경쟁·객단가 분석", "분기"),
        ("열린데이터광장", "서울시 상권분석서비스(점포-상권)", "서울신용보증재단", "위험도·경쟁(점포수·개폐업)", "분기"),
        ("열린데이터광장", "서울시 상권분석서비스(길단위인구-상권)", "서울신용보증재단", "유동인구 피크·마케팅 타게팅", "분기"),
        ("열린데이터광장", "서울시 상권분석서비스(상권변화지표-상권)", "서울신용보증재단", "위험도 2×2 매트릭스", "분기"),
        ("열린데이터광장", "서울시 상권분석서비스(직장인구-상권)", "서울신용보증재단", "5차원 입지 — 직주비율", "분기"),
        ("열린데이터광장", "서울시 상권분석서비스(영역-상권)", "서울신용보증재단", "상권↔행정동 매핑(전 기능)", "수시"),
        ("열린데이터광장", "서울시 문화행사 정보", "서울특별시", "문화행사 알림·메뉴 시즌", "수시"),
        ("열린데이터광장", "행정동 단위 서울 생활인구(내국인)", "서울특별시", "5차원 입지·쿠폰 피크타임", "일"),
        ("공공데이터포털", "국세청_사업자등록정보 진위확인 및 상태조회 서비스", "국세청", "온보딩 사업자번호 검증", "실시간"),
        ("공공데이터포털", "기상청_단기예보 조회서비스", "기상청", "날씨 기반 액션 보정", "실시간"),
    ]
    for i, (prov, ds, org, use, per) in enumerate(rows):
        y = 358 + i * 58
        if i % 2 == 0:
            _rect(s, PX(96), PX(y - 5), PX(1728), PX(54), fill=WHITE)
        seoul = prov == "열린데이터광장"
        _txt(s, PX(96), PX(y), PX(150), PX(42), prov, size=10, bold=True,
             color=BLUE if seoul else MUTE, anchor=MSO_ANCHOR.MIDDLE)
        _txt(s, PX(254), PX(y), PX(720), PX(42), ds, size=10.5, color=INK, anchor=MSO_ANCHOR.MIDDLE)
        _txt(s, PX(990), PX(y), PX(200), PX(42), org, size=10, color=SLATE, anchor=MSO_ANCHOR.MIDDLE)
        _txt(s, PX(1200), PX(y), PX(410), PX(42), use, size=10, color=SLATE, anchor=MSO_ANCHOR.MIDDLE)
        _txt(s, PX(1620), PX(y), PX(120), PX(42), per, size=10, color=MUTE, anchor=MSO_ANCHOR.MIDDLE)
    _txt(s, PX(96), PX(948), PX(1728), PX(28),
         "공공데이터 10종 결합 — 경진대회 결합 활용 가점(+2) 요건 충족. 교통분야 제외. 추출일 2026-04. ※ 가게 검색은 카카오 로컬 장소검색(카카오 개발자 API) 별도 사용.",
         size=11.5, bold=True, color=GREEN)
    footer(s, 6, "전 외부 호출은 backend/app/services 에서 재시도·대체(retry+fallback) 패턴으로 처리")
    ...
    feature_slide(
        ...
        data="서울시 상권분석서비스(추정매출-상권·점포-상권·상권변화지표-상권·길단위인구-상권).",
    ...
    feature_slide(
        ...
        data="위험 신호 · 매칭된 지원사업(마감 임박) · 서울시 문화행사 정보를 종합.",
    ...
    feature_slide(
        ...
        data="서울시 상권분석서비스(추정매출·점포·길단위인구·직장인구-상권) · 행정동 단위 서울 생활인구(내국인).",
    ...
    feature_slide(
        ...
        data="서울시 상권분석서비스(길단위인구-상권 — 시간대·요일·성별·연령 → 피크 타임·타깃) · 추정매출-상권(객단가 비교).",
    ...
    feature_slide(
        ...
        data="상권 업종 구성(다양성지수 — 서울시 상권분석서비스 기반) · 서울시 문화행사 정보 · 계절·학사 일정.",
