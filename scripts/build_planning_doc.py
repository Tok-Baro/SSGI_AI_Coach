...
def build():
    ...
    contrib = [
        ...
        ("재사용 파이프라인", "상권코드↔행정동 매핑·분기 캐싱·업종 코드명 정규화를 모듈화 — 다른 서비스도 같은 방식으로 서울 데이터 활용."),
    ...
    footer(s, 5, "라이브 배포: ssgi-ai-coach.vercel.app — 31개 API 라우트 구동 중")
    ...
    layers = [
        ...
        ("② RAG 검색 레이어", "환각 차단", GREEN,
         "지원사업 공고를 text-embedding-3-small 로 임베딩 → ChromaDB(코사인)에 인덱싱. 사장님 업종·매출·사업기간을 쿼리로 top-k 매칭. \"왜 맞는지\" 근거 문장도 함께."),
        ("③ GPT-4o 생성 레이어", "사장님 언어", BLUE,
         "매일 1줄 액션 · AI 종합진단 · 마케팅/메뉴 전략. 시스템 프롬프트에 \"업종 분기 가이드(6대 업종군 KPI·채널)\" + \"손실 프레이밍 어법\" 임베드. 추정치는 amber \"AI 추정\" 라벨 강제."),
        ("④ 가드레일 레이어", "정직성", AMBER,
    ...
    feature_slide(
        ...
        ai="GPT-4o. 시스템 프롬프트에 6대 업종군 분기 가이드(업종별 다른 KPI·채널) + 손실 프레이밍 어법 임베드. 같은 가게에 같은 액션 반복 방지.",
    ...
    feature_slide(
        ...
        ai="GPT-4o (deep-report 프롬프트). 업종 분기 + 사장님 화법. \"이번 주 1가지\"로 우선순위를 좁힘. 추정 매출·효과는 amber \"AI 추정\" 라벨 의무.",
    ...
    feature_slide(
        ...
        ai="GPT-4o (marketing-strategy 프롬프트). 업종별 채널 매트릭스(배민·인스타·네이버·카카오 단골) — 업종 안 맞는 헛조언 차단. ROI 수치는 \"가설\" 라벨, 결제 데이터 아님.",
    ...
    _rect(s, PX(96), PX(878), PX(1728), PX(82), fill=BLUEBG, radius=0.04)
    _txt(s, PX(124), PX(898), PX(1672), PX(46),
         "+ 대시보드 어디서나 음성 질문(플로팅 마이크) → 답변 + 해당 화면으로 자동 이동.",
         size=15, bold=True, color=BLUE)
    footer(s, 17, "frontend/src/app — 메인 4 라우트 + insights 4탭 + onboarding / 라이브 시연 가능")
    ...
    footer(s, 18, "전 화면 라이브 호출 — 백엔드 31개 API 라우트 / 시연 시 실물 데모 가능")
    ...
    rm = [("Q1", "결제 모듈 안정화 · 카카오페이 정기결제"), ("Q2", "MAU 확대 · D7 리텐션 25%+ 도전"),
          ("Q3", "지원사업 DB 자동 수집 · 업종 확장"), ("Q4", "1,000 MAU · Pre-A 라운드 검토")]
